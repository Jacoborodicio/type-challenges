...
function toDetailsInnerText(text, locale) {
    return `${(0, locales_1.t)(locale, `details.${text}`)}`;
}
...
function quizNoToBadges(ids, quizzes, locale, absolute = false) {
    return ids
...
function getAllTags(quizzes, locale) {
    const set = new Set();
...
function getQuizzesByTag(quizzes, locale, tag) {
